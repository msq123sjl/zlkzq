        ...
        if (mouseEvent->button() == Qt::LeftButton) {
            if (obj == ui->labCh0) {
                ...
                if (!obj->property("noinput").toBool() && objName != "frmMainWindow"
                        && objName != "frmInputWindow" && objName != "qt_edit_menu" && objName!="qt_scrollarea_viewport") {
                    ShowPanel();
                }
            }
...
void frmInput::focusChanged(QWidget *oldWidget, QWidget *nowWidget)
{
    ...
    if (nowWidget != 0 && !this->isAncestorOf(nowWidget)) {
        ...
        if (currentPosition == "center") {
            ...
            pos_width=pos.x();
            pos_height=pos.y()+2;
            if((pos_width+frmWidth) > 800)
                pos_width=((800-frmWidth)/2);
            if(pos_height+frmHeight>480)
                pos_height=pos_height-frmHeight-nowWidget->height()-2;
            if(pos_height<0)pos_height=0;
            this->setGeometry(pos_width, pos_height, frmWidth, frmHeight);
        }
    }
}
